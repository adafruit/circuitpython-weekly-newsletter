...
Welcome to the latest Python on Microcontrollers newsletter! Another busy week leading up to the holidays. Apple has finally performed some bug fixes for loading UF2 files onto microcontrollers but work remains with hex files. The Adafruit PyLeap application now makes it easier to load code onto your woreless microcontroller. Grab a free PDF copy of the Raspberry Pi Handbook 2023 to check out the latest on their boards. And check out all the projects this week, some very imaginative ones - *Ed.*
