...
## The Third Party CircuitPython Online IDE Editor Gains AI Callable Skills
...
CircuitPythonista River Wang has updated their web-based CircuitPython Online IDE to 2.4, and has written a tutorial on a full AI agent workflow with the IDE - [Adafruit Playground Note](https://adafruit-playground.com/u/River_Wang/pages/coding-circuitpython-with-ai-agent-the-easiest-setup) and [IDE Site circuitpy.dev](https://circuitpy.dev). Via [X](https://x.com/River___Wang/status/2075092027096662368).

> "To be clear, the IDE itself doesn't contain any AI. Instead, it exposes a set of tools, implemented as JavaScript functions, that an AI agent can call. These functions serve as the bridge between the IDE and the agent. This means the agent also needs to live inside the browser. The agent we're going to use is Claude in Chrome."
