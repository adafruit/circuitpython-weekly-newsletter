...
Microsoft Garage has posted another upgrade for their project Device Simulator Express! This one uses a VS code extension to program microcontollers with/without the device. The original program supported the Adafruit Circuit Playground Express. Now there is additional support for the BBC micro:bit and the Adafruit CLUE board with CircuitPython.
