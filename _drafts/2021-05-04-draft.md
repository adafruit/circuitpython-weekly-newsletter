...
Adafruit has stocked the [official MicroPython PyBoards](https://www.adafruit.com/?q=pyboard&sort=BestMatch) from Damien for a while. They have also made direct donations to MicroPython to support Damien’s work, as well as made and share contributions based on the CircuitPython core on MicroPython. Now, with GitHub sponsors, Adafruit just provided a $5k sponsorship. The MicroPython goal is $5k a month in sponsors, so 1 full month is on Adafruit. If you use MicroPython, please consider doing a monthly or one-time gift as well. Companies / orgs can now sponsor, so this was Adafruit sponsoring.
